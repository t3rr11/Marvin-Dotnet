  ...
  fields: [
    {
      ...
      data: [
        `progressions[${Progressions.Rankings.IRON_BANNER}].currentProgress`,
        `progressions[${Progressions.Rankings.IRON_BANNER}].currentResetCount`,
      ],
      resetInterval: 10000,
    },
  ],
...
export const generateLeaderboard = async (
  ...
  try {
    sortedPlayers = guildPlayers.sort((a, b) => {
      const a_current_progress = a.progressions[Progressions.Rankings.IRON_BANNER]?.currentProgress || 0;
      const b_current_progress = b.progressions[Progressions.Rankings.IRON_BANNER]?.currentProgress || 0;
      const a_resets = (a.progressions[Progressions.Rankings.IRON_BANNER]?.currentResetCount || 0) * 10000;
      const b_resets = (b.progressions[Progressions.Rankings.IRON_BANNER]?.currentResetCount || 0) * 10000;
      return Number(b_current_progress + b_resets) - Number(a_current_progress + a_resets);
    });

    embed.setTitle(`Top ${size} Seasonal Iron Banner Rankings`);
    embed.setDescription(
      `[Click to see full leaderboard](https://marvin.gg/leaderboards/${msgInt.guild.id}/iron_banner/)\n` +
        '```' +
