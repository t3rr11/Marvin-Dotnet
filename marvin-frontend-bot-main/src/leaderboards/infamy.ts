  ...
  fields: [
    {
      ...
      data: [
        `progressions[${Progressions.Rankings.INFAMY}].currentProgress`,
        `progressions[${Progressions.Rankings.INFAMY}].currentResetCount`,
      ],
      resetInterval: 10000,
    },
  ],
...
export const generateLeaderboard = async (
  ...
  try {
    sortedPlayers = guildPlayers.sort((a, b) => {
      const a_current_progress = a.progressions[Progressions.Rankings.INFAMY]?.currentProgress || 0;
      const b_current_progress = b.progressions[Progressions.Rankings.INFAMY]?.currentProgress || 0;
      const a_resets = (a.progressions[Progressions.Rankings.INFAMY]?.currentResetCount || 0) * 10000;
      const b_resets = (b.progressions[Progressions.Rankings.INFAMY]?.currentResetCount || 0) * 10000;
      return Number(b_current_progress + b_resets) - Number(a_current_progress + a_resets);
    });

    embed.setTitle(`Top ${size} Seasonal Infamy Rankings`);
    embed.setDescription(
      `[Click to see full leaderboard](https://marvin.gg/leaderboards/${msgInt.guild.id}/infamy/)\n` +
        '```' +
        `Rank: Infamy (Resets) - Name` +
        `\n\n` +
